...
public class Crypt {
  ...
  public String encrypt(String value) {
    try {
      ...
      String macEncoded = Mac.make("HmacSHA256", ivEncoded + encrypted, key);
  ...
  public String decrypt(String encrypted) {
    try {
      ...
      return new String(
        cipher.doFinal(
          payload.get("value").asText().getBytes(StandardCharsets.ISO_8859_1)
        ),
        StandardCharsets.ISO_8859_1
      );
    } catch (Exception e) {
  ...
  public boolean vaild(ObjectNode payload) {
    if (!payload.has("iv") || !payload.has("mac") || !payload.has("value")) {
      return false;
    }
